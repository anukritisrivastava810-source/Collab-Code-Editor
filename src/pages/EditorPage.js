...
const EditorPage = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      socketRef.current.on(
        ACTIONS.WRITE_ACCESS_REQUESTED,
        ({ requesterSocketId, userName, message }) => {
          toast(
            (t) => (
              <div>
                <b>{userName}</b> requests write access.
                <br />
        ...
        setOpenFiles((prev) => {
          if (prev.length === 0) {
            ...
            if (root && root.children && root.children.length > 0) {
              const firstFileId = root.children.find(
                (id) => fs[id]?.type === "file",
              );
              if (firstFileId) {
  ...
  const handleRenameNode = useCallback(
    (nodeId, newName) => {
      socketRef.current?.emit(ACTIONS.FS_RENAME_NODE, {
        roomId,
        nodeId,
        newName,
      });
  ...
  async function handleUploadFiles(e) {
    ...
    toast.success(
      `Uploaded ${fileCount} file${fileCount !== 1 ? "s" : ""}${folderNodes ? ` in ${folderNodes} folder${folderNodes !== 1 ? "s" : ""}` : ""}`,
    );
  }
        ...
        <div className="activity-bar desktop-only">
          <div className="activity-bar-top">
            <div className="activity-logo">
              <span className="logo-collab">C</span>
              <span className="logo-ce">E</span>
            </div>
            <button
            ...
            >
              ...
              {clients.length > 0 && (
                <span className="activity-badge">{clients.length}</span>
              )}
            </button>
          </div>
              ...
              <>
                ...
                >
                  ...
                  >
                    👥 CONNECTED{" "}
                    {isAdmin ? (
                      <span className="panel-admin-tag">Admin</span>
                    ) : (
                      ""
                    )}
                  </span>
                  ...
                  <div className="clientsList">
                    {clients.map((client) => (
                      <Client
                        ...
                        iAmAdmin={isAdmin}
                        onTogglePermission={() =>
                          togglePermission(client.socketId, client.canWrite)
                        }
              ...
              <div className="editor-empty-inner">
                ...
                {canWrite && (
                  <p className="editor-empty-hint">
                    Use + buttons in the explorer to create files & folders
                  </p>
                )}
              </div>
            </div>
